...
public class MultiSourceFetchPushDown {
    ...
    private static final Supplier<Set<Reference>> REFSET_SUPPLIER = new Supplier<Set<Reference>>() {
        @Override
        public Set<Reference> get() {
            return new HashSet<>();
        }
    };
    ...
    private void process() {
        ...
        statement.querySpec().outputs(new ArrayList<>(ctx.inputs.keySet()));
        ...
        if (statement.remainingOrderBy().isPresent()) {
            ListIterator<Symbol> iter = statement.remainingOrderBy().get().orderBySymbols().listIterator();
            while(iter.hasNext()){
    ...
    private class Context {
        LinkedHashMap<Symbol, InputColumn> inputs = new LinkedHashMap<>(statement.requiredForQuery().size());
        int idx = 0;

        private Symbol allocate(Symbol symbol) {
            InputColumn input = inputs.get(symbol);
            if (input != null) {
                return input;
            }
            if (statement.requiredForQuery().contains(symbol)) {
                input = new InputColumn(idx, symbol.valueType());
                inputs.put(symbol, input);
                return input;
            }
            return symbol;
        }
    ...
    private static class InputMappingVisitor extends ReplacingSymbolVisitor<Context> {

        private static final InputMappingVisitor IN_PLACE = new InputMappingVisitor(true);

        private InputMappingVisitor(boolean inPlace) {
            super(inPlace);
        }

        @Override
        protected Symbol visitSymbol(Symbol symbol, Context context) {
            return context.allocate(symbol);
        }
    }
